...
public class PostRequests extends RequestUtils {
    ...
    @Step("Посылаем POST запрос для авторизации на сервер: {0}")
    public Response postRequestForAuthorization(String path, String body, Map<String, String> cookies) {
        Response response = given().config(cfg).cookies(cookies).contentType("application/x-www-form-urlencoded")
        ...
        attachJson("Тело запроса", body);
        attachJson("Тело ответа", gsonFormatter(response));
        return response;
    }

    @Step("Посылаем POST запрос на сервер: {0}")
    public Response postRequest(String path, Object body) {
        Response response = given().config(cfg).cookies(cookieMap).contentType("application/json").with().body(gson.toJson(body)).post(generateURI(path));
        logger.info("URL: " + generateURI(path).toString());
        logger.info("HTTP STATUS: " + response.getStatusCode());
        logger.info("RESPONSE: " + response.asString());
        Assert.assertEquals(response.getStatusCode(), 200, "Статус отввета отличается от \"200\"");
        attachJson("Тело запроса", gson.toJson(body));
        attachJson("Тело ответа", gsonFormatter(response));
